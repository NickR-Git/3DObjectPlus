...
"use strict"
{
  ...
  self.C3.Plugins.Mikal_3DObject.Acts = {
    ...
    LoadModelFromTemplate(uid) {
      if (!this.instanceModel) {
        if (this.debug) console.warn("LoadModelFromTemplate only works on instance models")
        return
      }
      const template = this.runtime.GetInstanceByUID(uid)
      if (!template) {
        if (this.debug) console.warn("LoadModelFromTemplate template uid not found", uid)
        return
      }
      ...
      if (!templateInst.loaded) {
        if (this.debug) console.warn("LoadModelFromTemplate: template not loaded, instance uid, template uid", this.uid, uid)
        return
      }
