      ...
      if (!globalThis.vertexShaderGPUSkinningEnabledXYZ) {
        ...
        function GetDefaultVertexShaderSource_WebGL2(useHighP) {
          ...
          return [
            ...
            `        highp float emissiveStr_dbg = clamp(uEmissiveFactor.r + uEmissiveFactor.g + uEmissiveFactor.b, 0.0, 1.0);`,
            `        vVertexLighting = (vNormal + 1.0) * 0.5 * (1.0 - emissiveStr_dbg);`,
            ...
            `        highp float emissiveStr = clamp(uEmissiveFactor.r + uEmissiveFactor.g + uEmissiveFactor.b, 0.0, 1.0);`,
            `        vVertexLighting = clamp(sumColor*vColor.xyz, vec3(0.0), vec3(1.0)) * (1.0 - emissiveStr);`,
            `    }`,
            `}`,
          ].join("\n")
        }
        ...
        if (shader) {
          ...
          shader.glslWebGL2 = shader.glslWebGL2.replace(
            ...
            "if (uEnvNormalMap > 0.5) {\nhighp vec2 normNUV = (vTex - srcOriginStart) / (srcOriginEnd - srcOriginStart);\nhighp vec2 scaledNUV = (normNUV * uNormalTexScale + uNormalTexOffset) * (srcOriginEnd - srcOriginStart) + srcOriginStart;\nworldSpaceNormal = worldSpaceNormal + uEnvNormalScale * normalColor(scaledNUV, uNormalSampler);\n} else if (uTextureNormal > 0.0 && uEnvMapEnable < 0.5) {\nworldSpaceNormal = worldSpaceNormal + uTextureNormal * normalColor(vTex, samplerFront);\n}"
          )
          shader.glslWebGL2 = shader.glslWebGL2.replace(
            "lowp vec4 tex = texture(samplerFront, tiledUV);",
            `highp vec2 normUV = (tiledUV - srcOriginStart) / (srcOriginEnd - srcOriginStart);
    highp vec2 scaledUV = (normUV * uTexScale + uTexOffset) * (srcOriginEnd - srcOriginStart) + srcOriginStart;
    lowp vec4 tex = texture(samplerFront, scaledUV);
    if (uEnvMapEnable > 0.5) {
        highp vec3 envCamPos = -transpose(mat3(uEnvCamMat)) * vec3(uEnvCamMat[3][0], uEnvCamMat[3][1], uEnvCamMat[3][2]);
        highp vec3 envViewDir = normalize(pos - envCamPos);
        highp vec3 envWorldNorm = normalize(worldSpaceNormal);
        highp vec3 envWorldR = reflect(envViewDir, envWorldNorm);
        highp vec3 envViewR = mat3(uEnvCamMat) * envWorldR;
        highp vec2 envFinalUV = vec2(envViewR.x * 0.5 + 0.5, envViewR.y * 0.5 + 0.5);
        tex = vec4(texture(samplerFront, envFinalUV).rgb, tex.a);
    }
    worldSpaceNormal = worldSpaceNormal * (1.0 - emissiveStr);`
          )
          shader.glslWebGL2 = shader.glslWebGL2.replace("highp vec3 ambientColorL = uAmbientColor;", "highp float emissiveStrength = uEmissiveFactor.r + uEmissiveFactor.g + uEmissiveFactor.b;\n\thighp float emissiveStr = clamp(emissiveStrength, 0.0, 1.0);\n\thighp vec3 emissiveColor = uEmissiveFactor;\n\thighp vec3 ambientColorL = uAmbientColor * (1.0 - emissiveStr);")
          ...
          runtime._iRuntime.addEventListener("afterprojectstart", () => { if (globalThis._3DObjectDebug) console.warn("[3DObject] shader mikal_frag_light-8 not found") }, { once: true })
        }
      }
    }
